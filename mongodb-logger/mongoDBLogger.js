...
export class MongoDBRemoteLogger {
  constructor(config) {
    this.config = {
      uri: config.uri, // MongoDB Atlas 連線字串
      dbName: config.dbName || 'app_logs',
      collectionName: config.collectionName || 'response_logs',
  ...
  addToQueue(logEntry) {
    ...
    if (shouldUpload) {
      ...
      console.log(`📝 日誌已加入佇列 (${this.uploadQueue.length}/${this.config.batchSize})`);
  ...
  startAutoSync() {
    ...
    console.log(`⚙️ 自動同步已啟動 (間隔: ${this.config.syncInterval / 1000}秒)`);
  }
