...
0 3 * * 1 curl -X DELETE http://0.0.0.0:3002/mongo/cleanup/90
```
...
```bash
# 每週一和週四凌晨 3:00 清理
0 3 * * 1,4 curl -X DELETE http://0.0.0.0:3002/mongo/cleanup/60
```
...
```bash
# 每天凌晨 3:00 清理
0 3 * * * curl -X DELETE http://0.0.0.0:3002/mongo/cleanup/30
```
...
# MongoDB 日誌自動清理
# 每週一凌晨 3:00 清理 90 天前的日誌
0 3 * * 1 curl -X DELETE http://0.0.0.0:3002/mongo/cleanup/90 >> /var/log/mongo-cleanup.log 2>&1
...
const CLEANUP_URL = "http://0.0.0.0:3002/mongo/cleanup";
...
```bash
# 手動執行一次看看
/bin/sh -c "curl -X DELETE http://0.0.0.0:3002/mongo/cleanup/90"
