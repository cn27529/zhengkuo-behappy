...
RECEIPT_NUMBER="26040017"  # 替換為實際的合併打印編號
RECORD_IDS="[85,86,89]"  # 使用 id 85,86 的記錄進行合併測試
VOID_REASON="TEST作廢合併打印" # 作廢原因
...
echo ""
echo "-- 檢查 joinRecordDB（收據欄位應為 NULL）"
echo "SELECT id, receiptNumber, receiptIssued, receiptIssuedAt, receiptIssuedBy, receiptId FROM joinRecordDB WHERE receiptId = (SELECT id FROM mergedReceiptsDB WHERE receiptNumber = '$RECEIPT_NUMBER');"
echo "-----------------------------------------------"
