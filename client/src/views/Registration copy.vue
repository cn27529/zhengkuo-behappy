        ...
        <div class="form-grid">
          <div class="form-group address-row">
            ...
            <input
              ...
              required
              :readonly="isViewMode"
              :class="{ 'readonly-field': isViewMode }"
            />
          </div>
  ...
  setup() {
    ...
    const loadFormForEdit = async (formId) => {
      try {
        ...
        const success = await registrationStore.loadFormForEdit(formId);
        if (success) {
          ElMessage.success("表單載入成功，現在可以編輯");
        } else {
          ElMessage.error("載入表單失敗");
          goBack();
        }
      } catch (error) {
        console.error("載入表單錯誤:", error);
        ElMessage.error("載入表單時發生錯誤");
        goBack();
      } finally {
    ...
    // 加载表单进行查看
    const loadFormForView = async (formId) => {
      try {
        submitting.value = true;
        const success = await registrationStore.loadFormForView(formId);
        if (success) {
          ElMessage.success("表單載入成功");
        } else {
          ElMessage.error("載入表單失敗");
          goBack();
        }
    ...
    const updateForm = async () => {
      ...
      submitting.value = true;
      try {
        const result = await registrationStore.updateRegistration();
        if (result.success) {
    ...
    const addBlessingPerson = () => {
      ...
      registrationStore.addBlessingPerson();
    };

    const removeBlessingPerson = (id) => {
      if (isViewMode.value) return;
      registrationStore.removeBlessingPerson(id);
    };

    const addAncestor = () => {
      if (isViewMode.value) return;
      registrationStore.addAncestor();
    };

    const removeAncestor = (id) => {
      if (isViewMode.value) return;
      registrationStore.removeAncestor(id);
    };

    const addSurvivor = () => {
      if (isViewMode.value) return;
      registrationStore.addSurvivor();
    };

    const removeSurvivor = (id) => {
      if (isViewMode.value) return;
      registrationStore.removeSurvivor(id);
    };

    const copyBlessingAddress = () => {
      if (isViewMode.value) return;
      registrationStore.copyBlessingAddressToSalvation();
    };
