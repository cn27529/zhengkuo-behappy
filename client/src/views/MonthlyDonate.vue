    ...
    <div class="settings-section" style="margin-bottom: 1.5rem; display: none;">
      <el-card>
        ...
        <div class="settings-content">
          <p>每月基本單位金額：<strong>{{ monthlyUnitPrice }}元</strong></p>
          <p class="settings-hint">💡 提示：贊助金額必須是 {{ monthlyUnitPrice }} 元的倍數</p>
        </div>
      </el-card>
    </div>
      ...
      <div v-else>
        ...
        <div class="table-container">
          ...
          >
            ...
            >
              <template #default="{ row }">
                <div class="month-cell" :class="{ 'has-donate': row.months[month.yearMonth]?.length > 0 }">
              ...
              <template #default="{ row }">
                <div class="donate-stats">
                  <div class="stat-item">
                    ...
                    <span class="stat-value">{{ row.totalAmount.toLocaleString() }}</span>
                  </div>
                  <div class="stat-item">
      ...
      >
        ...
        <el-form-item label="贊助金額" prop="amount">
          <el-input-number
            ...
            style="width: 200px;"
          />
          <span class="form-hint">（必須是 {{ monthlyUnitPrice }} 的倍數）</span>
          
          <div class="amount-info" v-if="newDonator.amount > 0">
            <p>可贊助月份數：<strong>{{ calculateMonthCount(newDonator.amount) }}</strong> 個月</p>
            <p>每月金額：<strong>{{ monthlyUnitPrice }}</strong> 元</p>
          </div>
        </el-form-item>
        ...
        <el-form-item label="選擇月份" prop="selectedMonths">
          <div class="month-selection">
            ...
            <div class="month-selection-actions">
              ...
              <el-button @click="clearAllMonths" size="small">
                清空
              </el-button>
              <el-button 
            ...
            <div class="selection-info">
              <p>已選擇：<strong>{{ newDonator.selectedMonths.length }}</strong> 個月</p>
              <p>可選擇：<strong>{{ calculateMonthCount(newDonator.amount) }}</strong> 個月</p>
            </div>
          </div>
        </el-form-item>
      ...
      >
        <el-form-item label="贊助金額" prop="amount">
          ...
          <div class="amount-info" v-if="newDonateItem.amount > 0">
            <p>可贊助月份數：<strong>{{ calculateMonthCount(newDonateItem.amount) }}</strong> 個月</p>
          </div>
        </el-form-item>
        ...
        <el-form-item label="選擇月份" prop="selectedMonths">
          <div class="month-selection">
            ...
            <div class="selection-info">
              <p>已選擇：<strong>{{ newDonateItem.selectedMonths.length }}</strong> 個月</p>
              <p>可選擇：<strong>{{ calculateMonthCount(newDonateItem.amount) }}</strong> 個月</p>
              <p>可用月份：<strong>{{ availableMonthsForDonator.length }}</strong> 個</p>
            </div>
          </div>
        </el-form-item>
      ...
      <div v-if="selectedDonator" class="donator-detail">
        <div class="detail-header">
          ...
          <div class="donator-stats">
            <el-statistic title="總贊助金額" :value="selectedDonator.totalAmount" suffix="元" />
            <el-statistic title="總贊助月份" :value="selectedDonator.totalMonths" suffix="個月" />
          </div>
        </div>
        ...
        <div class="detail-content">
          ...
          <el-table :data="selectedDonator.donateItems" style="width: 100%">
            <el-table-column prop="donateItemsId" label="項目ID" width="180">
              <template #default="{ row }">
                <span class="font-mono">{{ row.donateItemsId.substring(0, 8) }}...</span>
              </template>
            </el-table-column>
            <el-table-column prop="price" label="金額" width="100" align="right">
              <template #default="{ row }">
                {{ row.price.toLocaleString() }} 元
              </template>
            </el-table-column>
            <el-table-column prop="months" label="贊助月份" min-width="200">
              <template #default="{ row }">
                <div class="months-list">
                  <el-tag 
                    v-for="month in row.months.slice(0, 5)" 
                    :key="month"
                    size="small"
                  >
                    {{ formatMonth(month) }}
                  </el-tag>
                  <el-tag v-if="row.months.length > 5" size="small">
                    +{{ row.months.length - 5 }}個月
                  </el-tag>
                </div>
              </template>
            </el-table-column>
          ...
          <h4 style="margin-top: 20px;">📊 月份分佈</h4>
          <div class="month-distribution">
            <div class="month-grid">
              <div 
                ...
                :class="{ 'has-donate': selectedDonator.months[month.yearMonth]?.length > 0 }"
              >
                <div class="month-label">{{ month.display.substring(5) }}</div>
                <div class="month-status">
                  <template v-if="selectedDonator.months[month.yearMonth]?.length > 0">
                    <el-tooltip 
                      :content="getMonthTooltip(selectedDonator.months[month.yearMonth])" 
                      placement="top"
                    >
                      <span class="donate-indicator">✅</span>
                    </el-tooltip>
...
const availableMonthsForDonator = computed(() => {
  ...
  const occupiedMonths = monthlyDonateStore.getDonatorMonths(selectedDonator.value.name);
  return monthColumns.value.filter(month => !occupiedMonths.includes(month.yearMonth));
});
...
const isMonthDisabled = (yearMonth) => {
  // 檢查月份是否已被選擇
  return newDonator.selectedMonths.includes(yearMonth) && 
...
const handleMonthSelectForItem = () => {
  ...
  if (newDonateItem.selectedMonths.length > maxMonths) {
    newDonateItem.selectedMonths = newDonateItem.selectedMonths.slice(0, maxMonths);
    ElMessage.warning(`最多只能選擇 ${maxMonths} 個月份`);
  }
};
...
const handleAddDonateItem = async () => {
  ...
  try {
    ...
    const isValid = await addDonateItemFormRef.value.validate().catch((error) => {
      console.error("表單驗證失敗:", error);
      return false;
    });
...
const deleteDonateItem = async (donator, item) => {
  try {
    ...
    if (donate) {
      const itemIndex = donate.donateItems.findIndex(i => i.donateItemsId === item.donateItemsId);
      if (itemIndex !== -1) {
        donate.donateItems.splice(itemIndex, 1);
        
        // 如果沒有其他贊助項目，刪除整個贊助記錄
        if (donate.donateItems.length === 0) {
          const donateIndex = allDonates.value.findIndex(d => d.donateId === donate.donateId);
          if (donateIndex !== -1) {
